...
static void find_obj(u8 *argv0) {
    ...
    *slash = '/';
    ...
    if (!access(tmp, R_OK)) {
      ...
      return;
        ...
        FATAL("Cannot access 'libDFSanPass.so'");
    }
  ...
  FATAL("Unable to find 'libDFSanPass.so'");
}

static void check_type(char *name) {
  if (!strcmp(name, "san-clang++")) {
    is_cxx = 1;
  }
  if(getenv("USE_DFSAN")){
    use_dfsan = 1;
...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    SAYF("\n"
         ...
         "  CC=%s/san-clang ./configure\n"
         "  CXX=%s/san-clang++ ./configure\n\n"
