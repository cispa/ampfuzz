...
pub fn start_forkcli() {
    match env::var(defs::FORKSRV_SOCKET_PATH_VAR) {
        Ok(socket_path) => {
            ...
            socket
                .set_read_timeout(Some(Duration::from_micros(config::RESPONSE_TIME_LIMIT * config::TIME_LIMIT_TRACK_FACTOR * 2)))
                .expect("Couldn't set read timeout");
            socket
                .set_write_timeout(Some(Duration::from_micros(config::RESPONSE_TIME_LIMIT * config::TIME_LIMIT_TRACK_FACTOR * 2)))
